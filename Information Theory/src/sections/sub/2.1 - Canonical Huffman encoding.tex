...
\begin{document}
    ...
    Before we proceed, let us observe something: there exist optimal codes 
        that are not produced by any Huffman tree, for instance the tree 
        on the right in \Cref{fig:nonHuffman}, corresponds to no Huffman, yet it's optimal.
    ...
    Building the \lstinline{num} and \lstinline{symb} array is trivial,
        ...
        \lstinline{fc[l] = fc[l + 1] + num[l + 1] << 1}. The pseudo-code in 
        Procedure \ref{proc:pseudoFC} uses a bottom-up approach, hence the tree built is 
        unbalanced to the left.
        
        \subfile{../../TikZ/Figure 4 - FC procedure}
        ...
        \begin{example*}
            Assume we want to compute the Canonical Huffman encoding for the tree 
                ...
                and \lstinline{symb = \{g, b, c, e, f, d\}}. We now proceed building the 
                \lstinline{fc} array. By using the procedure we have \lstinline{fc[2] = 0}
                    while \lstinline{fc[1] = (fc[2] + num[2]) << 1 = 2}. Note that in this 
                    case the shift operation is ternary. The tree we obtain, 
                    symmetric to that of \Cref{fig:exampleHuffman}, is shown below.
                    \subfile{../../TikZ/Figure - Example of Canonical Huffman encoding}
        \end{example*}

    To conclude our discussion, we now provide the pseudo code to decode a Canonical 
        Huffman encoded string. Recall that the decoder has access to both \lstinline{fc}
        and \lstinline{symb}; therefore, we can simply read bits until the value 
        is less than \lstinline{fc[l]}, for all \(l\). The Procedure \ref{proc:decodeCanonical}
        summarizes the procedure.
        \subfile{../../TikZ/Figure 5 - Canonical Huffman decoding}
\end{document}
