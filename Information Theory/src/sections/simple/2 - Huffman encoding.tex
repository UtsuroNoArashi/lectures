...
\begin{document}
    The first method to build optimal codes was discovered by D. A. Huffman,
        as solution problem posed by his lecturer Robert Fano\footnotemark.
    ...
    \begin{enumerate}
        \item In not available, compute the symbols probabilities by scanning the txst.
        \item Sort the symbols according to their probabilities, non-increseingly.
        \item Make sure that the number of source symbols is of the form \(d + \alpha(d - 1)\),
    ...
    In addition to the tree overhead, we have to consider the possibility in 
        which the tree gets corrupted. In the next few sections we consider some 
        variation to the plain Huffman encoding, that try to fix these issues.
    ...
    % \subfile{../sub/2.1 - Adaptive Huffman encoding}
\end{document}
