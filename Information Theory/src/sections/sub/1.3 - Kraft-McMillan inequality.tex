...
\begin{document}
    We now consider an interesting result about UD codes: the Kraft-McMillan inequality.
        Formally, the Kraft-McMillan inequality stastes that a code \(\C*\) over an alphabet 
        of size \(d\), with set of codewords lengths \(L\) is UD, if and only if 
        \[
        ...
        \begin{description}
            ...
            \item [\(\implies\)] Let \(l_{1}, \ldots, l_{n}\) be such that 
                \[
                    \sum_{i = 1}^{n} d^{-l_{i}} \le 1.
                \]
                Denote by \(n_{j}\) the number of codewords of lenght \(j\).
