...
\begin{document}
    Non-singular codes are not enough. 
        It may happen that the sequence of bit in analysis may be factored in
        as concatenation of distinc codewords,
        one again making the decoding process tedious. 

    In other words, what we are looking for are codes for which any sequece of codewords
        corresponds to at most one sequece of source symbol.
        We call such code \gls{ud} codes.
        
    Hence the questions: how do we construct UD codes? 
        And how do we determine if a given code is UD? Here we answer the second question,
    ...
    and \(w\). Additionaly, we align each \(w\) to the row of the corresponfing \(\alpha/\beta\).
    \clearpage 
