...
\begin{document}
    As for Huffman encoding, the arithmetic encoding procedure just discussed
        ...
        In the present section we discuss how we can achive adaptability.

    The idea is the same as that of adaptive Huffman, but instead of 
        updating the tree, we update the known probabilities.
        Algorithmically speaking, we begin by assuming all symbol have the same probability;
        that is, the \(n\) subintervals have the same width.
        As before, once a symbol is read we select the corresponding subinterval,
        and additionally we increase that symbol probability. The selected 
    ...
    \begin{example*}
        ...
        encoding \(S\) as \([\sfrac{43}{180}, \sfrac{11}{45}]\).
        \subfile{../../TikZ/Figure - Example of adaptive arithmetic encoding.tex}        
    \end{example*}

    The decoding step is simmetrycal: instead of updating the probabilities once a 
        symbol is read, these are updated each time a new one is decoded.
        Apart from these caveats, both the encoding and decoding procedures 
